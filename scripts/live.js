export default class Live {
    constructor() {
        this.universeWidth = 20;
        this.universeHeight = 20;
        this.fillStyle = '#d6d6d6';
        this.squareStyle = '#ffd100';
        this.universePadding = 1;
        this.lineWidth = 1;
        ...
        for (let h = 0; h < this.universeHeight; h++) {
            ...
            for (let w = 0; w < this.universeWidth; w++) {
                ...
                if (this.ctx) {
                    ...
                    this.ctx.fillRect(w * this.squareWidth + this.lineWidth, h * this.squareHeight + this.lineWidth, this.squareWidth - this.lineWidth, this.squareHeight - this.lineWidth);
                    this.ctx.stroke();
                }
            }
    ...
    clearSquare(x, y) {
        if (this.ctx) {
            this.ctx.beginPath();
            this.ctx.fillStyle = this.fillStyle;
            this.ctx.fillRect(x * this.squareWidth + this.lineWidth, y * this.squareHeight + this.lineWidth, this.squareWidth - this.lineWidth, this.squareHeight - this.lineWidth);
        }
        this.universe[y][x] = 0;
    }
    drawSqare(x, y) {
    ...
    check() {
        ...
        this.universe = this.universe.map((row, rowInd) => {
            ...
            return row.map((square, index) => {
                ...
                // UP
                if (this.universe[up][left] !== 0)
                    neighbor++;
                if (this.universe[up][right] !== 0)
                    neighbor++;
                if (this.universe[up][index] !== 0)
                    neighbor++;
                // MIDDLE
                if (this.universe[rowInd][left] !== 0)
                    neighbor++;
                if (this.universe[rowInd][right] !== 0)
                    neighbor++;
                // DOWN
                if (this.universe[down][left] !== 0)
                    neighbor++;
                if (this.universe[down][right] !== 0)
                    neighbor++;
                if (this.universe[down][index] !== 0)
                    neighbor++;
                // LIVE OR DEAD
                if (square === 1) {
